...
generate_summary() {
...

    {
        ...
        echo "诊断模式: $MODE"
        echo ""
        if [ "$MODE" = "auto" ] && [ -f "$OUTPUT_DIR/SUMMARY.txt" ]; then
...
main() {
    ...
    if [ -x "$SCRIPT_DIR/check_tools.sh" ]; then
    ...
    if [ -x "$SCRIPT_DIR/collect_info.sh" ]; then
    ...
    case "$MODE" in
        auto)
            log_info "运行自动检测..."
            if [ -x "$SCRIPT_DIR/quick_diagnosis.sh" ]; then
