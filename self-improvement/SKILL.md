...
description: 捕获学习、错误和更正以实现代理持续改进。当以下情况发生时必须触发：(1) 命令或操作意外失败，(2) 用户更正Claude回答（"不，那错了..."、"实际上..."），(3) 用户请求不存在的功能，(4) 外部 API 或工具失败，(5) 意识到知识过时或不正确，(6) 发现针对重复任务的更好方法。在主要任务之前会自动回顾学习内容。
---
