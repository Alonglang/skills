---
name: xlsx
description: "任何时间以电子表格文件为主要输入或输出时，必须使用此技能。支持打开、读取、编辑、修复现有的 .xlsx、.xlsm、.csv 或 .tsv 文件；从头创建新电子表格；在表格格式之间转换；清理格式化混乱的表格数据。当用户按名称或路径引用电子表格文件时，即使随意提及（如\"我的下载中 xlsx\"），并希望对其执行操作，必须立即触发此技能。可交付物必须是电子表格文件。"
