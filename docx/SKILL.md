---
name: docx
description: "每当用户想要创建、读取、编辑或操作 Word 文档（.docx 文件）时，使用此技能。触发因素包括：任何提及 'Word doc'、'word document'、'.docx'，或请求生成具有格式化（如目录、标题、页码或信头）的专业文档。亦用于从 .docx 文件中提取或重新组织内容，在文档中插入或替换图像，在 Word 文件中执行查找和替换，使用修订和注释，或将内容转换为精美的 Word 文档。如果用户要求作为 Word 或 .docx 文件的"报告"、"备忘录"、"信函"、"模板"或类似的可交付物，请使用此技能。不要用于 PDF、电子表格、Google Docs 或与文档生成无关的一般编码任务。"
