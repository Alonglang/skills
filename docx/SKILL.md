---
name: docx
description: "每当用户想要创建、读取、编辑或操作 Word 文档（.docx 文件）时，必须使用此技能。触发因素包括：任何提及 'Word doc'、'word document'、'.docx'、'Word文档'，或请求生成具有格式化（如目录、标题、页码或信头）的专业文档。亦用于从 .docx 文件中提取或重新组织内容，在文档中插入或替换图像，在 Word 文件中执行查找和替换，使用修订和注释，或将内容（Markdown、文本）转换为精美的 Word 文档。如果用户要求输出为 Word 或 .docx 格式的报告、备忘录、信函、模板或任何类似可交付物，请立即触发此技能。不要用于 PDF、电子表格、Google Docs 或与文档生成无关的一般编码任务。"
...
### 验证
创建文件后，对其进行验证。如果验证失败，请解包、修复 XML 并重新打包。
```bash
...
// 相同的 reference = 继续 (1,2,3 然后 4,5,6)
// 不同的 reference = 重新开始 (1,2,3 然后 1,2,3)
```

### 表格

**关键：表格需要双重宽度**——在表格上设置 `columnWidths`，并在每个单元格上设置 `width`。没有两者，表格在某些平台上无法正确渲染。
...
// US Letter 带有 1" 边距：12240 - 2880 = 9360 DXA
...
使用带有 `type: SectionType.NEXT_COLUMN` 的新部分强制列中断。
...
- **始终使用 DXA 设置表格 `width`** - 绝不使用 `WidthType.PERCENTAGE`（在 Google Docs 中中断）
...
- **绝不使用表格作为分隔符/规则** - 单元格具有最小高度并呈现为空框（包括在页眉/页脚中）；在 Paragraph 上使用 `border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: "2E75B6", space: 1 } }` 代替。对于双列页脚，使用制表符（请参阅制表符部分），而不是表格
- **TOC 仅需要 HeadingLevel** - 标题段落上没有自定义样式
- **覆盖内置样式** - 使用确切的 ID："Heading1"、"Heading2" 等
- **包括 `outlineLevel`** - 目录所必需（H1 为 0，H2 为 1 等）
...
python scripts/office/unpack.py document.docx unpacked/
```
提取 XML，漂亮打印，合并相邻的 run，并将智能引号转换为 XML 实体（`&#x201C;` 等），以便它们在编辑中存活。使用 `--merge-runs false` 跳过 run 合并。

### 步骤 2：编辑 XML

编辑 `unpacked/word/` 中的文件。请参阅下面的 XML 参考了解模式。

**使用 "Claude" 作为作者**进行修订和注释，除非用户明确要求使用不同的名称。

**直接使用 Edit 工具进行字符串替换。不要编写 Python 脚本。**脚本引入了不必要的复杂性。Edit 工具确切显示正在替换的内容。

**关键：对新内容使用智能引号。**当使用撇号或引号添加文本时，使用 XML 实体生成智能引号：
```xml
<!-- 使用这些实体以获得专业的排版 -->
...
**添加注释**：使用 `comment.py` 处理多个 XML 文件的样板（文本必须是预先转义的 XML）：
```bash
...
```
然后在 document.xml 中添加标记（请参阅 XML 参考中的注释）。
...
- 在带有空格的 `<w:t>` 上缺少 `xml:space="preserve"`
...
- **替换整个 `<w:r>` 元素**：当添加修订时，将整个 `<w:r>...</w:r>` 块替换为 `<w:del>...<w:ins>...` 作为兄弟。不要在 run 内注入修订标记。
- **保留 `<w:rPr>` 格式化**：将原始 run 的 `<w:rPr>` 块复制到你的修订运行中以保持粗体、字体大小等。
...
- **空白**：在前导/尾随空格的 `<w:t>` 中添加 `xml:space="preserve"`
...
**删除整个段落/列表项** - 当从段落中删除所有内容时，也将段落标记标记为已删除，以便它与下一段合并。在 `<w:pPr><w:rPr>` 中添加 `<w:del/>`：
```xml
<w:p>
  <w:pPr>
...
运行 `comment.py`（请参阅步骤 2）后，将标记添加到 document.xml。对于回复，使用 `--parent` 标志并将标记嵌套在父标记内。
