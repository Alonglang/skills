...
| 优先级路由 | `usage-based-routing-v2`（+优先级配置） | 不同用户/密钥的优先级调度 |
...
基于请求优先级的调度，适合多租户场景中区分高优先级付费用户和低优先级普通用户。在 `usage-based-routing-v2` 策略上启用优先级参数：
...
router_settings:
  ...
  routing_strategy_args:
    ...
    default_priority: 10    # 默认优先级（数字越小越优先）
```

客户端请求通过 `metadata.priority` 指定优先级（或在创建密钥时预设）：
...
**优先级处理逻辑：**
- 优先级 1 的请求始终优先于优先级 10 的请求获取路由资源
- 高优先级不绕过 Provider 的 RPM/TPM 限制，但在多部署竞争时会优先分配
- 未指定 priority 的请求使用 `default_priority`
